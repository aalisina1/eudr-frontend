// forked from design-sync lib/bundle.mjs — adds a browser `process` shim banner to
// the IIFE build. This Next.js app's components pull Next runtime internals
// (next/navigation, next/link) and the API client, which read process.env.__NEXT_*
// / process.versions / process.nextTick at module eval; in the browser preview
// `process` is undefined and the bare reference crashes the whole IIFE before
// window.<GLOBAL> is assigned. The banner only polyfills a runtime global — it does
// NOT change the export shape, the @ds-bundle header, or the manifest contract.
...
// Minimal browser polyfill so Next/node-style `process.*` reads resolve to safe
// falsy/no-op values instead of throwing `process is not defined`.
const PROCESS_SHIM_BANNER =
  "globalThis.process||(globalThis.process={env:{},argv:[],version:'',versions:{},platform:'browser',browser:true,nextTick:function(f){Promise.resolve().then(f)},cwd:function(){return '/'}});";
...
export async function bundleToIife({ entry, globalName, nodePaths, out, tsconfig }) {
  ...
  try {
    buildResult = await build({
      ...
      format: 'iife',
      globalName,
      // Browser `process` polyfill — see PROCESS_SHIM_BANNER note at top.
      banner: { js: PROCESS_SHIM_BANNER },
