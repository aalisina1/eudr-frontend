...
function DueDiligencePageInner() {
  ...
  // `src/app/(dashboard)/sourcing/[id]/page.tsx`) takes over this whole
  ...
  // here as `/submissions?status=SUBMITTED` — seed the status toolbar
  ...
  return (
    <div className="space-y-6">
      ...
      <DataTable<DueDiligenceStatement>
        ...
        exportable
        rowKey={(stmt) => stmt.id}
        onRowClick={(stmt) => router.push(`/submissions/${stmt.id}`)}
